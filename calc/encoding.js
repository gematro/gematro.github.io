...
function toggleEncodingMenu(updateEncMenu = false, clearValues = false) {
	if (!encodingMenuOpened || updateEncMenu == true) {
		...
		o += '<td><input type="text" autocomplete="off" spellcheck="false" placeholder="abcdefghijklmnopqrstuvwxyz" value="'+prevAlphStr+'" class="colSlider" id="encAlphabetBox" style="width: 300px;" oninput="encPrevAlphStr = document.getElementById(&quot;encAlphabetBox&quot;).value"'+SylParamChkState+'></td>'
		o += '</tr><tr>'
		o += '<td><span class="colLabelSmall">Vowels</span></td>'
		o += '<td><input type="text" autocomplete="off" spellcheck="false" placeholder="aeiouy" value="'+prevVowStr+'" class="colSlider" id="encVowelsBox" style="width: 300px;" oninput="encPrevVowStr = document.getElementById(&quot;encVowelsBox&quot;).value"'+SylParamChkState+'></td>'
		o += '</tr><tr>'
		o += '<td><span class="colLabelSmall">Exclude</span></td>'
		o += '<td><input type="text" autocomplete="off" spellcheck="false" placeholder="qzjx" value="'+prevExcStr+'" class="colSlider" id="encExcludeBox" style="width: 300px;" oninput="encPrevExcLetStr = document.getElementById(&quot;encExcludeBox&quot;).value"'+SylParamChkState+'></td>'
		o += '</tr>'
		o += '</table>'
...
function encodeGematria(n_phr, encCiphValsArr, encCiphIndArr, exclude = []) {
	...
	if (encodingMode == 0) { // use syllables
		...
		for (i = 0; i < n_phr; i++) {
			...
			solution = (optGemSubstitutionMode) ? solution.trim()+' [<a href="https://wordsmith.org/anagram/anagram.cgi?anagram='+solution.trim()+'&language=english&t=0&d=3&include=&exclude=&n=&m=&a=n&l=n&q=n&k=1&source=adv" target="_blank" rel="noopener noreferrer nofollow">anagram</a>]' : solution.trim()
