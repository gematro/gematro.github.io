...
function gemCalcModeLabel(curCipher) {
	if (!curCipher.multiCharacter && optGemMultCharPos && curCipher.LetterCount > 0) return ' - x1 ... x'+curCipher.LetterCount
	else if (!curCipher.multiCharacter && optGemMultCharPosReverse && curCipher.LetterCount > 0) return ' - x'+curCipher.LetterCount+' ... x1'
	return ""
}
...
function updateWordBreakdown(impName = breakCipher, impBool = false, chartUpd = true) { // false - preview temporary (hover), true - lock breakdown to a specific cipher
	...
	if (curCipher.cArr.indexOf(1488) > -1 || curCipher.cArr.indexOf(1575) > -1) {
		...
		leftToRightBreak = curCipher.multiCharacter ? true : false
	...
	if (curCipher.sumArr.length > 0) {
		...
		if (optWordBreakdown == true && !calcLongBreakdownWidth(curCipher) ) { // character limit, calculated even if out of screen bounds
			...
			if (optCompactBreakdown == true) {
				...
				o += curCipher.sumArr.reduce(getSum)
				o += ' </span>'
...
function updateCipherChartGemCard(impName = breakCipher) {
	...
	o += '<td colspan="' + curCiphercArr.length + '">'
	o += '<font style="font-size: 150%; font-weight: 500; '+curCiphCol+'">' + curCipher.cipherName + '</font>'
	o += '</td></tr><tr>'
	...
	for (var x = 0; x < curCiphercArr.length; x++) {
		...
		o += curCipher.multiCharacter ? '<td class="ChartChar" font style="'+curCiphCol+'">' + curCiphercArr[x] + '</td>' : '<td class="ChartChar" font style="'+curCiphCol+'">' + String.fromCodePoint(curCiphercArr[x]) + '</td>'
	}
	if (curCiphercArr.length % 2 == 1) { o += '<td class="ChartChar" font style="'+curCiphCol+'"></td>' } // empty character cell to make even rows
	o += '</tr><tr>'
	for (y; y < x; y++) {
